# ============================================================
# Fabric Notebook — Bulk Create DAX Measures via TOM API
# ============================================================
...
measures = [
    ...
    ("FinalTable", "Avg_Enrolls_Per_BDA", 'DIVIDE([Enrolls_Per_BDA], [Active_BDAs], 0)', "8. BDA Performance", "#,##0.0"),
    ...
    ("FinalTable", "Revenue_by_Tier", 'CALCULATE(SUM(FinalTable[sale_value]), NOT ISBLANK(FinalTable[bda_tier]))', "9. BDA Tier", "₹#,##0"),
    ...
    ("FinalTable", "Revenue_PM", 'CALCULATE([Revenue], DATEADD(FinalTable[activity_month_start], -1, MONTH))', "11. MoM", "₹#,##0"),
    ...
    ("FinalTable", "Revenue_MoM%", 'DIVIDE([Revenue] - [Revenue_PM], [Revenue_PM], 0)', "11. MoM", "0.0%"),
    ...
    # ── 13. Data Quality ─────────────────────────────────────
    ("FinalTable", "System_Leads", 'CALCULATE(DISTINCTCOUNT(FinalTable[lead_id]), FinalTable[Is_System_Activity] = 1)', "13. Data Quality", "#,##0"),
    ...
    ("CallDetail", "Dials_per_Lead", 'DIVIDE([Dials], [Dialled_Leads], 0)', "2. Calling Ratios", "#,##0.0"),
...
with fabric.create_tom_server(readonly=False) as tom:
    ...
    for table_name, name, expression, folder, fmt in measures:
        try:
            ...
            # Remove existing measure if present
    ...
    print("\nSaving model...")
    model.SaveChanges()
    print("✅ All measures saved!")

print(f"\nDone! {len(measures)} measures processed.")
